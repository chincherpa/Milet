...
public sealed partial class EingangsrechnungListViewModel : ObservableObject
{
    private readonly IBelegService _belegService;
    private readonly INavigationService _navigation;
    private readonly IDialogService _dialogService;

    public EingangsrechnungListViewModel(IBelegService belegService, INavigationService navigation, IDialogService dialogService)
    {
    ...
    {
        LaedtGerade = true;
        try { Belege = await _belegService.SucheAsync(BelegTyp.Eingangsrechnung, Suchtext); }
        catch (Exception ex) { await _dialogService.ZeigeFehlerAsync("Fehler beim Laden", ex.Message); }
        finally { LaedtGerade = false; }
    }

    [RelayCommand] private void Neu() => _navigation.Navigate<EingangsrechnungEditViewModel>(0);
    [RelayCommand] private void Bearbeiten() { if (Ausgewaehlt is { } beleg) _navigation.Navigate<EingangsrechnungEditViewModel>(beleg.Id); }

    [RelayCommand]
    private async Task LoeschenAsync()
    {
        if (Ausgewaehlt is not { } beleg) return;
        var bestaetigt = await _dialogService.BestaetigenAsync("Eingangsrechnung löschen", $"Eingangsrechnung '{beleg.BelegNummer}' wirklich löschen?");
