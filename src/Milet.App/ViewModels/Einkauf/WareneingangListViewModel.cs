...
public sealed partial class WareneingangListViewModel : ObservableObject
{
    private readonly IBelegService _belegService;
    private readonly INavigationService _navigation;
    private readonly IDialogService _dialogService;

    public WareneingangListViewModel(IBelegService belegService, INavigationService navigation, IDialogService dialogService)
    {
    ...
    {
        LaedtGerade = true;
        try { Belege = await _belegService.SucheAsync(BelegTyp.Wareneingang, Suchtext); }
    ...
    private async Task LoeschenAsync()
    {
        ...
        var bestaetigt = await _dialogService.BestaetigenAsync("Wareneingang löschen", $"Wareneingang '{beleg.BelegNummer}' wirklich löschen?");
