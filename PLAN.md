...
**Stand:** Phasen 0–2 sind implementiert und abgenommen. Phasen 3 (Lager+Lieferschein), 4 (Einkauf) und 5 (Finanzen+E-Mail) sind implementiert (Backend Build/Tests grün; manueller UI-Smoke-Test steht bei allen dreien noch aus; Phase 5 zusätzlich mit dem Vorbehalt, dass `Milet.App` in der Umsetzungssession mangels Windows kein einziges Mal gebaut werden konnte — Details in `STATUS.md`). **Nächster Schritt: vor Phase 6 (DATEV+Reporting) sollten die drei offenen Windows-Verifikationen (Phase 3/4/5) auf einer echten Windows-Maschine nachgeholt werden — Implementierungsplan für Phase 5 liegt vor in `docs/superpowers/plans/2026-08-27-phase5-finanzen-mail.md`.**
