...
**Stand:** Phasen 0–2 sind implementiert und abgenommen. Phasen 3 (Lager+Lieferschein), 4 (Einkauf), 5 (Finanzen+E-Mail), 6 (DATEV+Reporting), 7 (Admin+Härtung) und 8 (Gärtnerei/Kultur) sind implementiert (Backend Build/Tests grün; manueller UI-Smoke-Test steht bei allen sechs noch aus; Phasen 5–8 zusätzlich mit dem Vorbehalt, dass `Milet.App` in der jeweiligen Umsetzungssession mangels Windows kein einziges Mal gebaut werden konnte — Details in `STATUS.md`; Phasen 6, 7 und 8 haben Build/Tests/Migration real gegen einen containerisierten SQL Server statt nur übersprungener Integrationstests verifiziert, inkl. echter RBAC-Logins und AuditLog-Protokollierung in Phase 7 und einer real gegen eine Datenbank mit Vor-Phase-8-Bestandsdaten angewendeten Migration in Phase 8). Alle 8 Plan-Phasen sind damit backend-seitig umgesetzt.

Zusätzlich ist **Phase 9 (Lückenschluss)** backend-seitig abgeschlossen (`docs/superpowers/plans/2026-08-31-luecken-schliessen.md`, sechs Blöcke, alle real gegen containerisierten SQL Server verifiziert): Storno für Rechnung/Lieferschein/Wareneingang (neue `Gutschrift`-Belegart), Lagerbewegung trägt Grund und Benutzer, Skontokonten konfigurierbar, Login-Lockout, erzwungener Passwortwechsel, eine seit Phase 3 unverifizierte Race-Condition in der Beleg-Überleitung real reproduziert und behoben, Lieferadresse im Beleg editierbar, sowie Tests für zuvor ungetestete Services (`ZahlungService`/`MahnwesenService`/`AuditSaveChangesInterceptor`). Bewusst zurückgestellt: eine fachliche Gutschrift ohne Storno-Bezug (Retoure/Kulanz) und der DATEV-Export für Gutschriften. **Nächster Schritt: die sieben offenen Windows-Verifikationen (Phase 3/4/5/6/7/8/9) sollten auf einer echten Windows-Maschine nachgeholt werden, danach ist Milet inhaltlich feature-komplett gemäß diesem Plan** — Deployment-Anleitung liegt vor in `docs/deployment.md`, der Phase-8-Smoke-Test-Ablauf in `docs/smoke-tests.md`.
