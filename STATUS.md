...
**Domain:** `Beleg`-Partei-Erweiterung — `KundeId` wird nullable, neues `LieferantId` (Kunde XOR Lieferant, künftig per DB-Check-Constraint erzwungen) — als Basis für die drei neuen TPH-Subtypen `Bestellung`/`Wareneingang`/`Eingangsrechnung`. `BestellVorschlagService` (reine Domain-Logik: Artikel mit Bestand unter `Mindestbestand`, `VorschlagsMenge`-Berechnung).
