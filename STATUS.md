...
- **[Behoben, Phase 9 Block 9e, 2026-08-31]** Offene-Mengen-Prüfung in `BelegUeberleitungService` schützte
  ...
  konnten beide „nichts geliefert" sehen und beide committen. Der Verdacht stand seit Phase 3 unverifiziert
  im Risiko-Log; in dieser Session **real reproduziert** (`BelegUeberleitungRaceTests`, zwei parallele
  Teillieferungen à 6 Stück gegen einen Auftrag über 10 lieferten vor dem Fix tatsächlich 12 aus) und
  behoben: alle drei Lesestellen der Quellbelege (`UeberleitenAsync`/`UeberleitenMitAuswahlAsync`/
  `UeberleitenMehrereAsync`) laden den/die Quellbeleg(e) jetzt über `FromSqlInterpolated(... WITH (UPDLOCK,
  HOLDLOCK) ...)` statt per einfachem LINQ — das serialisiert zwei gleichzeitige Überleitungen desselben
  Quellbelegs; der zweite Aufruf sieht danach die bereits committete Folgemenge und lehnt korrekt ab statt
  über eine veraltete Momentaufnahme zu doppeln. Fund stammt ursprünglich aus einem parallel entstandenen,
